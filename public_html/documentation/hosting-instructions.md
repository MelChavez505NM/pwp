...
- ssh into the server
    - For Mac and Linux: `ssh root@swtaxandaccounting.dev`
    - For Windows: Set up a new PuTTY session
        ...
        - **Session**
            ...
            - Click Save
- create user
    - `useradd -m -s /bin/bash -g users -G sudo mchavez`
    - replace mchavez with your own
- create password
    - `passwd mchavez905`
- Move .ssh key file
    - `cp -r .ssh /home/mchavez905`
- change ownership of the .ssh directory
    - `cd /home/mchavez905`
    - `chown -R mchavez905:users .ssh/`


- Test Changes
    - `exit`
    - Mac/Linux: `ssh mchavez@swtaxandaccounting.com`
    - Windows:
        - **Session**
            - Click pwp
            - Click Load
...
- reboot
    - `sudo reboot`
- ssh back in
- `ssh mchavez@swtaxandaccounting.com`
...
- create a directory for your deployment
    - `sudo mkdir -p/var/www/swtaxandaccounting.com/html`
- fix permissions
    - `sudo chown -R $USER:users /var/www/swtaxandaccounting.com/html`
...
- `sudo vim /etc/nginx/sites-available/swtaxandaccounting.dev`
...
server {
    ...
    root /var/www/swtaxandaccounting.dev/html;
    index index.html index.htm index.nginx-debian.html;

    server_name swtaxandaccounting.dev www.swtaxandaccounting.dev;
...
- create a symlink to the sites-enabled directory
    - `sudo ln -s /etc/nginx/sites-available/swtaxandaccounting.dev /etc/nginx/sites-enabled/`
...
- Create a new ssh key on your server in the .ssh directory
    - `ssh-keygen -t ed25519 -C  "github Deploy Key" -f /home/mchavez/.ssh/github_deploy_key`
    - Don't enter a passphrase
        - hit enter twice
- Copy the public key into your authorized keys file
    - `cat /home/mchavez/.ssh/github_deploy_key.pub >> /home/mchavez/.ssh/authorized_keys`
...
- Copy the private key to your clipboard **(this is a very dangerous operation do not share this key with anyone )**
    ...
    - `cat /home/mchavez/.ssh/github_deploy_key`
  ...
  deploy:
    ...
    steps:
      ...
      - name: Deploy to server
        run: |
         ssh-keyscan -H swtaxandaccounting.dev >> ~/.ssh/known_hosts 
         scp -r ./dist/* gkephart@swtaxandaccounting.dev:/var/www/swtaxandaccounting.dev/html
```
